...
import { AnthropicMessagesUsage, convertAnthropicMessagesUsage } from './convert-anthropic-messages-usage';
...
function createCitationSource(
    ...
    // Handle web search result citations
    ...
    // Handle document citations (page_location and char_location)
...
export class AnthropicMessagesLanguageModel implements LanguageModelV3 {
    ...
    private async getArgs({
        ...
        if (
            ...
            if (!tools?.some((tool) => tool.type === 'provider' && tool.id === 'anthropic.code_execution_20250825')) {
                warnings.push({
                    type: 'other',
            ...
            switch (part.type) {
                ...
                case 'web_search_tool_result': {
                    if (Array.isArray(part.content)) {
                        ...
                        // Note: Sources are NOT added here - they come via citations_delta
                        // which properly interleaves them with the text that cites them
                    } else {
                        content.push({
                            type: 'tool-result',
                            toolCallId: part.tool_use_id,
