...
- **federation** _AnthropicFederationSettings_

  Authenticate with [Workload Identity Federation](#workload-identity-federation)
  instead of a static credential. Cannot be combined with `apiKey` or `authToken`.
...
With [Workload Identity Federation](https://platform.claude.com/docs/en/manage-claude/workload-identity-federation)
a deployed workload authenticates with the OIDC identity token it already has
(from Vercel, AWS, GitHub Actions, Kubernetes, ...) instead of a static API key.
The provider exchanges the identity token for a short-lived Anthropic access token,
sends it as a bearer token, caches it, refreshes it ahead of expiry,
and retries a request once with a fresh token if the API rejects the cached one.

Federation is used automatically when neither `apiKey` nor `authToken` is set,
`ANTHROPIC_API_KEY` is unset, and the `ANTHROPIC_FEDERATION_RULE_ID` and
`ANTHROPIC_ORGANIZATION_ID` environment variables are set.
Pass the `federation` setting to use it explicitly, or to supply the identity token from code:
...
const anthropic = createAnthropic({
  federation: {
    // the Vercel deployment's OIDC token, read from the request context
    identityToken: () => getVercelOidcToken(),
  },
});
```

The `federation` setting accepts the following options, each defaulting to the
environment variable the Anthropic SDKs read:

- **federationRuleId** _string_
...
- **identityToken** _string | () =&gt; string | Promise&lt;string&gt;_

  The OIDC identity token asserting the workload's identity, or a function returning it.
  The function is called on every exchange, so it can return a fresh token each time,
  e.g. by reading a projected service account token file.
  Defaults to the `ANTHROPIC_IDENTITY_TOKEN` environment variable.

A misconfigured or rejected exchange throws an `AnthropicFederationError`,
which carries the token endpoint's `statusCode` and `requestId`.
A denied exchange is logged with its reason in the Claude Console's workload identity authentication history.
